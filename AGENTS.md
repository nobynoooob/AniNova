...
- `AutoSkipMonitor(ipc_socket, resolver, on_skip=None, osd=True)` daemon thread: polls mpv `time-pos` every 0.5 s; triggers when `start <= pos < end` AND (`last_pos < start` crossed forward OR `pos - start <= 1.5` fresh entry) while not paused; seeks to `end`, OSD `show_text("Skipped Opening/Ending", 2000)`, calls `on_skip(end, skip_type)`. Re-arms once `pos >= end` or `pos < start` (rewind before the window re-triggers; mid-intro rewind does not). Exits after 6 consecutive dead polls or stop event. Intervals resolve lazily (late fetch still lands). `_owns_ipc` closes the client it created.
...
- `player.py`: `_start_auto_skip(ipc_socket, auto_skip)` starts the monitor beside the progress poller; both `_play_mpv` (single-rendition) and the multi-variant branch join it with a timeout after `proc.wait()`. `play_with_quality_fallback(..., auto_skip=None)` threads the config through.
